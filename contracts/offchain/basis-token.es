{
    ...
    //       signs a transaction spending the box and taking all funds (tokens and ERG).
    ...
    if (action == 0) {
      ...
      sigmaProp(
        selfPreserved &&
        tokenIdsPreserved &&
        selfOut.tokens(1)._2 >= SELF.tokens(1)._2 && // no reserve tokens can be taken out at initiation
        selfOut.value >= SELF.value && // no ERG can be taken out at initiation
      ...
      // (reserve tokens and ERG), destroying the reserve box. Creditors had
      // refundPeriod blocks to redeem. Owner's signature covers the whole
      // transaction, so the owner controls the destination of funds and tokens.
      sigmaProp(
