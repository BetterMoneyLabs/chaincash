...
\authorrunning{}
\titlerunning{Money Creation With Elastic Supply Via Trust And Blockchain Assets}
...
\keywords{blockchain, digital currency, peer-to-peer money}
...
by other peers becomes an individual choice. Similarly to spender-signed currencies, every spender is signing and backing every
...
blockchains experiment with algorithmic money issuance. As another option, we also have alternative (usually, local),
monetary systems, such as LETS~(local exchange trading systems~\cite{williams1996new}), timebanks, local government
currencies~(such as famous W\"orgl demurrage-based local currency back in 1930s~\cite{unterguggenbercer1934end}), and so on.
...
fiat monetary systems (in contrast with commodity money used before fiat, as well as alternative monetary systems) have
...
On the other hand, supply is disconnected from economic activity and so not elastic. Local currencies are usually considered
more fair in segniorage distribution than fiat currencies, they are often successfully boosting local economies, but,
in contrast with fiat and crypto-currencies, they are not global and rarely surviving in the long term due to loss of active supporters.
...
reserve to collective backing. At the same time, a newly issued note could be accepted by a peer without any backing provided, for example, if such a note is issued by a friend or a trusted charity. Every peer in the system has own individual rules
for accepting notes~(widely accepted standards may exist at the same time), which provides basis for elasticity of supply. We provide details in the next section.
...
From desiderata above, we may describe the protocol with three basic entities, a reserve, a note, and a receipt. Possible actions involving those entities are deposit into reserve, note redemption,
 receipt redemption, and note spending. It is possible to withdraw funds from a reserve by simply issuing a note, then spending with a signature
...
avoid having global state maintained on the blockchain, and note progression is well described by underlying UTXOs progression also. Another feature which is critical for feasible implementation is
possibility to manage signature chain efficiently. We use Ergo as a blockchain to implement \cc{} as UTXO transactional model as well as AVL+ trees support are making notes and reserves implementation feasible.
...
\begin{itemize}
  ...
  \item{} client software~(which we refer to as \cc{} Server), which interacts with the blockchain (in future, possibly, also a p2p network
  where notes are making progress off-chain). This software implements $a_i$ agent's functionality from the Section~\ref{sec-design},
...
Three on-chain contracts are implemented, namely, reserve, note and receipt contracts. Reserve contract locks ERG native tokens on top of Ergo blockchain and allows redeeming native or custom tokens when a note is presented. Note contract ensures that a note~(UTXO) under its control has proper history, that is, on every spending a valid signature of corresponding reserve owner is added. It is also allowing for a note to be split into two parts~(payment and change), as well as note redemption.
On redemption, where both reserve and note contracts are involved, an output with receipt contract is created, which contains history of ownership copied from the note input, as well as position of reserve redeemed in ownership chain and note's value. With receipt it is possible then to redeem against an earlier reserve~(reserve contract allows for that).
...
A local or even national government may issue notes and enforce their acceptance within its jurisdiction by enforcing economic agents to accept notes issued or spent by the government. As well as in a LETS implementation, enforced acceptance rules may vary, for example, there
could be a limit for amount of money a government may create.

Often local currencies are introducing redemption fee, to promote local usage. In \cc{}, similar goals can be achieved via modifying the reserve contract in a way that non-locals need to pay redemption fee while locals need not, alternatively, the note contract could be modified in a way that spending to non-local addresses incurs a fee. Local currencies are often associated with demurrage, after well-known W\"orgl experiment~\cite{unterguggenbercer1934end}. Demurrage could be implemented by modifying note contract.
...
The most appealing option, likely, has notes created and making progress off-chain, with on-chain reserves.
...
\begin{itemize}
  ...
  \item unlike native cryptocurrencies and algorithmic stablecoins, \cc{} provides elasticity of supply without enforcing
...
\begin{itemize}
  ...
  (collateralized debt positions) with different level of collateralization also should be priced differently. And like a DAI protocol is assigning the same price to
