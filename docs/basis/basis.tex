...
The mean of payment is an IOU note, which is signed by the issuer and also by a witnessing tracker. Our design allows for debt transferability, if issuer agrees on that, so if peer A issued debt to peer B, and peer B pays with it, fully or partially, to peer C, with A also co-signing, and then A owes C.
...
The paper is organized as follows. In Section~\ref{sec-design} we describe the design of \basis{} in details. Section~\ref{sec-security} analyzes its security properties. Section~\ref{sec-impl} provides implementation details. In Section~\ref{sec-adv} we provide analysis of advantages and drawbacks of the protocol.
