...
The paper is organized as follows. In Section~\ref{sec-design} we describe the design of \basis{} in detail. Section~\ref{sec-security} analyzes its security properties. Section~\ref{sec-impl} provides implementation details. In Section~\ref{sec-adv} we provide analysis of advantages and drawbacks of the protocol.
...
When an agent (the payer) wishes to create a debt obligation to another agent (the payee), it creates an IOU note containing the payer's signature, the payee's public key, the debt amount, and a timestamp. The note may be associated with an on-chain reserve contract that backs the debt, if such reserve does exist. Before the payee accepts the note, it may request witnessing from a \basis{} tracker. The tracker verifies the note, updates its local state to record the new debt relationship, and returns a tracker signature certifying that the debt is now tracked. The payee accepts the witnessed note as a valid claim against the payer.
